...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 ...
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False, same_mask=False, hybrid_ratio=0., isAnchor=False):
        super().__init__()
    ...
    def forward_encoder(self, x, timestamps, mask_ratio, mask=None):
        ...
        if self.isAnchor:
            x, mask, ids_restore = self.anchor_random_masking(x, timestamps, mask_ratio, mask=mask) 
        else:
