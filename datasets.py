...
class DatasetLMDBList(SatelliteDataset):
    # [[lmdb_path_0, key_path_0]...[lmdb_path_3, key_path_3]]
    def __init__(self, lmdb_list):
        super().__init__(in_c=3)
        self.data_type = "lmdb"
        self.lmdb_list = lmdb_list
        self.lmdb_env_list, self.lmdb_key_list = self._init_lmdb()
        # self.keys_path = "/share/home/thuzjx/data/city_metainfo.pkl"
        # assert self.lmdb_path, "Error: lmdb_path is Empty!!" 
        self.keys_list, self.data_len, self.start_point = self._get_image_keys_from_lmdb()
        # self.keys = self._get_image_keys_from_lmdb()
        # self.data_len = len(self.keys)
        self.totensor = transforms.ToTensor()
    ...
    def _init_lmdb(self):
        env_list = []
        key_list = []
        for i in range(len(self.lmdb_list)):
            env_list.append(lmdb.open(self.lmdb_list[i][0], readonly=True, lock=False, readahead=False, meminit=False))
            key_list.append(self.lmdb_list[i][1])
        return env_list, key_list
    ...
    def _get_image_keys_from_lmdb(self):
        ...
        key_len = 0
        for i in range(len(self.lmdb_list)):
            keys_path = self.lmdb_key_list[i]
            meta_info = pickle.load(open(keys_path, "rb"))
            keys = meta_info['keys']
            keys_list += keys
            start_point.append(key_len)
            key_len += len(keys)
        return keys_list, key_len, start_point

    def _read_img_lmdb(self, lmdb_id, currentkey, size=(3, 3, 224, 224) ):
        env = self.lmdb_env_list[lmdb_id]
        key = currentkey 
        with env.begin(write=False) as txn:
            buf = txn.get( key.encode() )
        img_flat = np.frombuffer(buf, dtype=np.uint8)
    ...
    def __getitem__(self, index):
        if self.data_type=='lmdb' and (self.lmdb_env_list is None):
            self._init_lmdb()
        lmdb_id = 9999
        for i in range(len(self.lmdb_list)):
            if index>=self.start_point[i]:
                lmdb_id = i
            else:
                break
        current_key = self.keys_list[index]
        images = self._read_img_lmdb(lmdb_id, current_key)
        # images = self._read_img_lmdb(currentkey=current_key)
        img_as_tensor_1 = torch.stack((self.totensor(images[0,:,:,:]), self.totensor(images[1,:,:,:]), self.totensor(images[2,:,:,:]) ),   dim=0 )#  3,3,224,224
        ge = []
        ge1 = self.geo_int2code(current_key.split('_')[4:12]) 
...
def build_fmow_dataset(is_train: bool, args, lmdb_list) -> SatelliteDataset:
    """
    ...
    if args.dataset_type == 'city_lmdb':
        ...
        dataset = DatasetLMDB(lmdb_path='/share/home/thuzjx/data/city.lmdb')
    elif args.dataset_type == 'lmdb_list':
        dataset = DatasetLMDBList(lmdb_list=lmdb_list) # 4*2 item
